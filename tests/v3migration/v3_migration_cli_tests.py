...
class v3MigrationTests(ArchesTestCase):
    ...
    def test_v3migration_001_create_v3_data_dir(self):
        ...
        management.call_command('v3', 'start-migration',
                                target=self.pkg,
                                overwrite=True
                                )
    ...
    def test_v3migration_002_generate_rm_configs(self):
        ...
        shutil.copytree(os.path.join(self.pkg_fixture, 'graphs', 'resource_models'),
                        os.path.join(self.pkg, 'graphs', 'resource_models'))

        # now run the management command
        management.call_command('v3', 'generate-rm-configs',
                                target=self.pkg
                                )
    ...
    def test_v3migration_003_generate_lookup_files(self):
        ...
        # not their content.
        management.call_command('v3', 'generate-lookups',
                                target=self.pkg,
                                overwrite=True
                                )
        ...
        management.call_command('v3', 'test-lookups',
                                target=self.pkg,
                                )
    ...
    def test_v3migration_004_convert_v3_skos(self):
        ...
        shutil.copytree(os.path.join(self.pkg_fixture, 'v3data', 'reference_data'),
        ...
        management.call_command('v3', 'convert-v3-skos',
                                target=self.pkg
                                )
    ...
    def test_v3migration_005_write_v4_relations_file(self):
        ...
        shutil.copytree(os.path.join(self.pkg_fixture, 'v3data', 'business_data'),
        ...
        # run the conversion command.
        management.call_command('v3', 'write-v4-relations',
                                target=self.pkg
                                )
    ...
    def test_v3migration_006_import_data_and_relations(self):
        ...
        all_models = [os.path.splitext(i)[0] for i in os.listdir(rm_dir) if
                      i.endswith('.json')]

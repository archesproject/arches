...
from arches.app.views.api import APIBase, Surveys, Resources
...
class APITests(ArchesTestCase):
    ...
    def setUp(self):
        ...
        with open(os.path.join('tests/fixtures/resource_graphs/unique_graph_shape.json'), 'rU') as f:
            ...
            self.unique_graph = Graph(json['graph'][0])
            self.unique_graph.save()

        with open(os.path.join('tests/fixtures/resource_graphs/ambiguous_graph_shape.json'), 'rU') as f:
            json = JSONDeserializer().deserialize(f)
            self.ambiguous_graph = Graph(json['graph'][0])
            self.ambiguous_graph.save()

        with open(os.path.join('tests/fixtures/resource_graphs/phase_type_assignment.json'), 'rU') as f:
            json = JSONDeserializer().deserialize(f)
            self.phase_type_assignment_graph = Graph(json['graph'][0])
            self.phase_type_assignment_graph.save()
    ...
    #     branch = resource_api.findBranch(graphtree, "http://www.cidoc-crm.org/cidoc-crm/P1_is_identified_by", jsonld_graph["http://www.cidoc-crm.org/cidoc-crm/P1_is_identified_by"])
    ...
    #     graphtree = self.unique_graph.get_tree()
    #     resource_api = Resources()
    #     with self.assertRaises(resource_api.DataDoesNotMatchGraphException) as cm:
    #         branch = resource_api.findBranch(graphtree, "http://www.cidoc-crm.org/cidoc-crm/P1_is_identified_by", incorrect_jsonld_graph["http://www.cidoc-crm.org/cidoc-crm/P1_is_identified_by"])
    ...
    #     resource_api = Resources()
    #     with self.assertRaises(resource_api.AmbiguousGraphException) as cm:
    #         branch = resource_api.findBranch(graphtree, "http://www.cidoc-crm.org/cidoc-crm/P1_is_identified_by", ambiguous_jsonld_graph["http://www.cidoc-crm.org/cidoc-crm/P1_is_identified_by"])
    ...
    def test(self):
        ...
        # ipdb.set_trace()
        with self.assertRaises(resource_api.AmbiguousGraphException) as cm:
            branch = resource_api.findBranch(graphtree, "http://www.cidoc-crm.org/cidoc-crm/P41i_was_classified_by", complex_jsonld_graph["http://www.cidoc-crm.org/cidoc-crm/P41i_was_classified_by"])

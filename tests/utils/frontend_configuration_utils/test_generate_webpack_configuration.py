...
class TestGenerateWebpackConfiguration(TestCase):
    def setUp(self):
        ...
        self.generate_webpack_configuration_function = self.webpack_configuration_module.generate_webpack_configuration
    ...
    def test_generate_webpack_configuration_returns_expected_payload_with_two_apps(self):
        arches_application_names = ["app_one", "app_two"]
        arches_application_paths = ["/arches_apps/app_one", "/arches_apps/app_two"]

        with patch.object(self.webpack_configuration_module, "list_arches_app_names", return_value=arches_application_names), \
             patch.object(self.webpack_configuration_module, "list_arches_app_paths", return_value=arches_application_paths):
        ...
        self.assertEqual(result["_comment"], "This is a generated file. Do not edit directly.")
        ...
        self.assertEqual(result["WEBPACK_DEVELOPMENT_SERVER_PORT"], self.development_server_port)
        self.assertEqual(result["SITE_PACKAGES_DIRECTORY"], sysconfig.get_path("purelib"))

    def test_generate_webpack_configuration_with_no_apps_returns_empty_mapping(self):
        with patch.object(self.webpack_configuration_module, "list_arches_app_names", return_value=[]), \
             patch.object(self.webpack_configuration_module, "list_arches_app_paths", return_value=[]):
            result = self.generate_webpack_configuration_function()

        self.assertEqual(result["ARCHES_APPLICATIONS"], [])
        self.assertEqual(result["ARCHES_APPLICATIONS_PATHS"], {})

    def test_generate_webpack_configuration_raises_when_app_names_and_paths_lengths_mismatch(self):
        with patch.object(self.webpack_configuration_module, "list_arches_app_names", return_value=["one", "two"]), \
             patch.object(self.webpack_configuration_module, "list_arches_app_paths", return_value=["/arches_apps/one"]):
            with self.assertRaises(ValueError):
                self.generate_webpack_configuration_function()

    def test_generate_webpack_configuration_site_packages_directory_is_an_existing_path(self):
        with patch.object(self.webpack_configuration_module, "list_arches_app_names", return_value=[]), \
             patch.object(self.webpack_configuration_module, "list_arches_app_paths", return_value=[]):
            result = self.generate_webpack_configuration_function()
        ...
        self.assertTrue(os.path.exists(site_packages_directory), f"Expected existing path, got {site_packages_directory}")

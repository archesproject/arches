...
class LabelBasedGraphTests(TestCase):
    ...
    node_1 = LabelBasedNode(name="name", value={VALUE_KEY: "value",},)

    node_2 = LabelBasedNode(name="name", value={VALUE_KEY: None,},)
    ...
    def test_add_node_no_previous_values(self):
        ...
        self.assertEqual(
            graph, {self.node_1.name: self.node_1.value},
        )
    ...
    def test_add_node_previous_value_list(self):
        ...
        self.assertEqual(
            graph, {self.node_1.name: [self.node_1.value, self.node_2.value]},
        )
    ...
    def test_from_tile_with_node_tile_reference(self, mock__build_graph):
        with mock.patch("arches.app.utils.label_based_graph.models.Node", return_value=None):
            LabelBasedGraph.from_tile(
                tile=mock.Mock(nodegroup_id=1), node_tile_reference=mock.Mock(),
            )

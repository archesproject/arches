...
3. In your project's `settings.py` file:
    ...
    - by default `ARCHES_NAMESPACE_FOR_DATA_EXPORT` is assigned the value `"http://localhost:8000/"`.
        If running your Django server elsewhere, overwrite `ARCHES_NAMESPACE_FOR_DATA_EXPORT` in your project's `settings.py` file.
    - by default `WEBPACK_DEVELOPMENT_SERVER_PORT` is assigned the value `9000`.
...
7. In a seperate terminal, navigate to root directory of the project ( you should be on the same level as `package.json`) 
...
2. in your project's `settings.py` file:
    ...
    7. by default `ARCHES_NAMESPACE_FOR_DATA_EXPORT` is assigned the value `"http://localhost:8000/"`.
        If running your Django server elsewhere, overwrite `ARCHES_NAMESPACE_FOR_DATA_EXPORT` in your project's `settings.py` file.
    8. by default `WEBPACK_DEVELOPMENT_SERVER_PORT` is assigned the value `9000`.
        If you want the webpack server to run on a different port, overwrite `WEBPACK_DEVELOPMENT_SERVER_PORT` in your project's `settings.py` file.
...
15. In a seperate terminal, navigate to root directory of the project ( you should be on the same level as `package.json`) 

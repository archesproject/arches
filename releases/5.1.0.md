...
Some features of version 5.0 graphs and resource data are not compatible with v5.1. This means that once you complete the upgrade, you will not be able to safely import 5.0 graphs and 5.0 resource data into a 5.1 system. Therefore, ***after upgrading be sure to export your graphs and data if you intend to reimport them or import them into a different 5.1 system***.
...
Version 5.1 requires you to use unique nodes names in a node group and between sibling nodes. After upgrading you may notice that some node names will have the nodeid appended to the end of the name. This was done to prevent any name conflicts. You can change the name to something else so long as the unique naming constraints are followed.

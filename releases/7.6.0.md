...
- 10804 Search has been re-architected to be totally component-based, now governed by `search-view` components. See [docs](https://arches.readthedocs.io/en/latest/developing/reference/search-components/) for more details.
...
- 11179 Add the ability to search if files do/do not exist, search on file size
- 11180 Add the ability to search if a geometry exists/does not exist.  Search on geometry type
...
14. Within your project, with your Python 3 virtual environment activated:
    ```
    python manage.py updateproject
    ```

    - Pay attention to notices about existing conflicting files that need manual reconciliation with the [updated project template](https://github.com/archesproject/arches/blob/stable/7.6.0/arches/install/arches-templates), if any. Some templates have placeholders with `{{ project_name }}` or `{{ project_name_title_case }}` that need to be replaced with your actual project name.

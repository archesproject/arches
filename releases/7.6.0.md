...
10. In settings.py:
    ...
    2. After `INSTALLED_APPS`, add this line to ensure "arches.app" (provider of core arches templates) always follows any Arches Applications:
        ...
        ```
    3. Update `WEBPACK_LOADER` to the following:
        ```
        WEBPACK_LOADER = {
            "DEFAULT": {
        ...
        ```
    4. Update `LOCALE_PATHS.append(...)` to `LOCALE_PATHS.insert(0, ...)`
        ```
        LOCALE_PATHS.insert(0, os.path.join(APP_ROOT, 'locale'))
        ```
    5. Add `"json"` to `FILE_TYPES` if you wish to use the JSON-LD bulk import module.
    6. Remove the `ARCHES_APPLICATIONS` setting.
    7. Replace:
        ```

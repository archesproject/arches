...
14. Run `python manage.py updateproject`. Pay attention to notices about existing conflicting files that need manual reconciliation with the [updated project template](https://github.com/archesproject/arches/blob/stable/7.6.0/arches/install/arches-templates), if any. Some templates have placeholders with `{{ project_name }}` or `{{ project_name_title_case }}` that need to be replaced with your actual project name.

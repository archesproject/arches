...
- 10121 Treat language codes as case-insensitive
...
1. Within your project, with your Python 3 virtual environment activated:
    ...
    This release of arches de-duplicates language codes that differ only in case,
    e.g. `en-us` versus `en-US`. This de-duplication only allows for variation in
    the `isdefault` column. If other columns vary, e.g. `name`, `scope`, or `direction`,
    then the migration will fail with a hint to bring the affected language records into
    agreement first.

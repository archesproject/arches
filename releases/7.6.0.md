...
15. If you do any custom development, update and reinstall the python development dependencies, and install the pre-commit hooks. (You can always ignore these hooks with `git commit --no-verify`.)
...
16. Navigate to the directory with your project's package.json file. Then run:
    ```
    npm install
    ```

17. Start your application server in a separate terminal if it's not already running.

18.  In the same terminal window where you ran `npm` ( on the same level as `package.json` ):
...
19. Test your project with deprecation warnings enabled.

    ```
    python -Wall::DeprecationWarning manage.py test --settings=tests.test_settings
    ```

20. Run system checks against your production settings.

    ```
    python manage.py check --deploy --settings=path.to.production.settings
    ```

21.  If you are running Arches on Apache, be sure to run:

...
7. Update the JavaScript dependencies and devDependencies
...
8. Start your application server in a separate terminal if it's not alreay running.

9. In the same terminal window where you ran `yarn` ( on the same level as `package.json` ), run `yarn start` or `yarn build_development`. This will generate your `media/build` directory.

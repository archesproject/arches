...
1. in your project's`settings.py` file:
    ...
    3. ensure `APP_NAME` has been defined
...
2. Copy the `arches/install/arches-templates/project_name/webpack/` directory from arches core directory to the root directory of the project ( it should be on the same level as `package.json`)
    ...
    - ensure that `PROJECT_ROOT_DIRECTORY` and `ARCHES_CORE_DIRECTORY` are pointing to the correct directories in your system in either `webpack/webpack-user-config.js`  or `webpack/webpack-meta-config.js` 

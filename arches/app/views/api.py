...
class SpatialView(APIBase):
    ...
    def validate_json_data_content(self, json_data, spatialviewid_identifier=None, is_post=False):
        ...
        if is_post:
            # POST requests should not have a spatialviewid in the JSON data as it is generated by the model
            if "spatialviewid" in json_data:
                return JSONErrorResponse(
                    _("Incorrect Spatialview json data"),
                    _("POST requests should not have a spatialviewid provided in the JSON data."),
                    status=400,
                )

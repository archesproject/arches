...
class ProjectManagerView(BaseManagerView):
    ...
    def get(self, request):
        ...
        for user in User.objects.filter():
            ...
            identities.append({'name': user.email or user.username, 'groups': ', '.join(groups), 'type': 'user', 'id': user.pk, 'last_login': get_last_login(user.last_login), 'default_permissions': set(default_perms), 'is_superuser':user.is_superuser, 'group_ids': group_ids})

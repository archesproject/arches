...
def save_to_tiles(userid, loadid):
    with connection.cursor() as cursor:
        saved = False
        try:
            log_event_details(cursor, "done|Disabling the triggers in the tile table...")
            cursor.execute("""
            ...
            """)
            log_event_details(cursor, "done|Saving the tiles...")
            ...
            if not saved:
                ...
                log_event_details(cursor, "done|Getting the statistics...")
                cursor.execute(
            ...
            log_event_details(cursor, "done|Reenabling the triggers in the tile table...")
            cursor.execute("""
        ...
        try:
            log_event_details(cursor, "done|Indexing...")
            ...
            log_event_details(cursor, "done|Updating the edit log...")
            cursor.execute(
                """
                    UPDATE edit_log e

...
class ImportSingleCsv(BaseImportModule):
    ...
    def validate(self,cursor):
        ...
        """
        rows = self.get_validation_result(cursor)
    ...
    def run_load_task(self, loadid, graphid, has_headers, fieldnames, csv_mapping, csv_file_name, id_label):
        with connection.cursor() as cursor:
            populated = self.populate_staging_table(cursor, loadid, graphid, has_headers, fieldnames, csv_mapping, csv_file_name, id_label)

            validation = self.validate(cursor)
    ...
    def populate_staging_table(self, cursor, loadid, graphid, has_headers, fieldnames, csv_mapping, csv_file_name, id_label):

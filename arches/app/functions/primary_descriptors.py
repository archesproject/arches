    ...
    def get_primary_descriptor_from_nodes(self, resource, config):
        ...
        try:
            if "nodegroup_id" in config and config["nodegroup_id"] != "" and config["nodegroup_id"] is not None:
                tiles = [
                    tile
                    for tile in self.foo_tiles
                    if tile.nodegroup_id == uuid.UUID(config["nodegroup_id"])
                    and tile.sortorder == 0
                    and tile.resourceinstance_id == resource.resourceinstanceid
                ]

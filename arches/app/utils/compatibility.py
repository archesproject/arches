...
def is_arches_compatible(min_arches=settings.MIN_ARCHES_VERSION, max_arches=settings.MAX_ARCHES_VERSION, target="project"):
    ...
    man_arches: a tuple with at least a major version of arches, but may also include a min and patch release e.g. (7,5,0)
    target: a description of what is being checked for compatibility

    """

    if min_arches is None or max_arches is None:
        logger.error(_("Either a minimum or maximum compatible Arches version is not specified. Unable to check {0} compatibility".format(target)))
        return

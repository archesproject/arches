...
def index_resources():
    ...
    for resource_type in resource_types:
        ...
        if resource_type[0] =='HERITAGE_RESOURCE_GROUP.E27':
            body = {
                'mappings': {
                    resource_type[0]: {
            ...
            try:
                se.es.indices.create(index='resource', body=body, ignore=400)
            except Exception as detail:
                print '%s: WARNING: failed to create resource index: %s \nException detail: %s\n' % (datetime.now(), body, detail)
                raise detail

...
class HtmlWriter(Writer):
    ...
    def get_graphids_with_export_template():
        ...
        for existing_graphid in existing_graphids:
            if HtmlWriter.has_html_template(existing_graphid):
                valid_graphs.append(existing_graphid)
    ...
    def load_html_template(self, graphid=None):
        template = None
        try:
            template = get_template(f"{graphid}.html")
        except TemplateDoesNotExist as e:
            pass
        return template
    ...
    def generate_html_files(self, resource_object_list=None):
        ...
        for gid in resource_object_list.keys():
            ...
            context = Context({"resources": resource_object_list[gid]})
            dest = StringIO()
            dest.write(template.render(context))
            
            files.append({"name": f"{str(GraphModel.objects.get(pk=gid))}.html", "outputfile": dest})
        
        return files

class HtmlWriter2(Writer):
    def __init__(self, **kwargs):
        super(HtmlWriter, self).__init__(**kwargs)

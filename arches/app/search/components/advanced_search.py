...
class AdvancedSearch(BaseSearchFilter):
    ...
    def view_data(self):
        ...
        searchable_nodes = models.Node.objects.filter(
            graph__isresource=True, graph__publication=True, datatype__in=searchable_datatypes, issearchable=True
        )
        resource_cards = models.CardModel.objects.filter(graph__isresource=True, graph__publication=True).prefetch_related("nodegroup")

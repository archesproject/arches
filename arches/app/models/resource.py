    ...
    def delete(self, user={}, index=True, transaction_id=None):
        ...
        if user != {}:
            ...
            if user_is_reviewer is False:
                ...
                resource_is_provisional = True if sum([len(t.data) for t in tiles]) == 0 or [any(t.data.values()) for t in tiles][0] == False else False
                if resource_is_provisional is True:
                    creator_id = int(EditLog.objects.filter(resourceinstanceid=self.pk, edittype='create').values('userid').first()['userid'])

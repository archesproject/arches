...
class Concept(object):
    ...
    def get_child_edges(
        ...
        child_valuetypes = ("','").join(
            child_valuetypes
            if child_valuetypes
            else models.DValueType.objects.filter(category="label").values_list(
                "valuetype", flat=True
            )
        )
        limit_clause = (
            " limit %s offset %s" % (limit, offset) if offset is not None else ""
        )
    ...
    def get_preflabel(self, lang=settings.LANGUAGE_CODE):
        ...
        ranked_labels = sorted(
            ranked_labels, key=lambda label: label["weight"], reverse=True
        )
    ...
    def bulk_index(self):
        ...
        if self.nodetype == "ConceptScheme":
            ...
            for topConcept in self.get_child_concepts_for_indexing(
                ...
                concept_docs.append(
                    se.create_bulk_item(
                        index="concepts", id=topConcept["id"], data=topConcept
                    )
                )
                for childConcept in concept.get_child_concepts_for_indexing(
                    ...
                    concept_docs.append(
                        se.create_bulk_item(
                            index="concepts", id=childConcept["id"], data=childConcept
                        )
                    )
...
def get_valueids_from_concept_label(label, conceptid=None, lang=None):
    ...
    def exact_val_match(val, conceptid=None):
        ...
        if conceptid is None:
            return {
                "query": {"constant_score": {"filter": {"terms": {"value": [val]}}}}
            }
        else:
            return {
                "query": {
    ...
    concept_label_results = se.search(
        index="concepts", body=exact_val_match(label, conceptid)
    )

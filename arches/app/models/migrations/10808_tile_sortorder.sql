    ...
    forward = """
        ...
        AS $BODY$
            DECLARE
                ...
                previous_resourceid uuid;
                previous_nodegroupid uuid;
                sortvalue int;
            BEGIN
                FOR staged_value, instance_id, legacy_id, tile_id, parent_id, nodegroup_id, passed, graph_id, source, op IN
                    (
                        ...
                        ORDER BY resourceid, nodegroup_depth, nodegroupid ASC
                    )
                LOOP
                    IF passed THEN
                        ...
                        IF op = 'update' THEN
                            ...
                            IF NOT FOUND THEN
                                IF previous_resourceid != instance_id OR previous_nodegroupid != nodegroup_id OR previous_resourceid is null OR previous_nodegroupid is null THEN
                                    SELECT COALESCE(max(sortorder), 0) FROM tiles INTO sortvalue WHERE resourceinstanceid = instance_id AND tileid = tile_id;
                                END IF;
                                INSERT INTO tiles(tileid, tiledata, nodegroupid, parenttileid, resourceinstanceid, sortorder)
                                    VALUES (tile_id, tile_data, nodegroup_id, parent_id, instance_id, sortvalue);
                        ...
                        previous_resourceid = instance_id;
                        previous_nodegroupid = nodegroup_id;
                    END IF;
                END LOOP;

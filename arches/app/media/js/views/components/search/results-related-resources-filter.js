define([
    ...
    'templates/views/components/search/results-related-resources-filter.htm',
], function(ko, arches, BaseFilter, resultsRelatedResourcesFilterTemplate) {
    var componentName = 'results-related-resources-filter';
    const viewModel = BaseFilter.extend({
        initialize: async function(options) {
            options.name = 'Results Related Resources Filter';
            ...
            if (componentName in query) {
                ...
                if (resourceTypeQuery.length > 0) {
                    resourceTypeQuery.forEach(function(type){
                        ...
                        this.getFilter('term-filter').addTag("Related "+type.name, this.name, type.inverted);
                    }, this);
        ...
        selectModelType: function(item){
            ...
            if(!!item){
                ...
                this.getFilter('term-filter').addTag("Related "+item.name+"(s)", this.name, inverted);
    ...
    return ko.components.register(componentName, {
        viewModel: viewModel,
        template: resultsRelatedResourcesFilterTemplate,
    });
});

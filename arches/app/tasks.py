...
@shared_task
def update_resource_instance_data_based_on_graph_diff(initial_graph, updated_graph, user_id):
    logger = logging.getLogger(__name__)
    ...
    try:
        ...
        notify_completion(_("Business has been updated in concurrence with publishing the latest model. {} Resource Instances updated.".format(resource_instance_count)), user)

    except Exception as e:
        logger.error(e)
        notify_completion(_("Business data update failed! Check the error logs for more information."), user)

...
_OP_PARAMS: dict[str, list[str]] = {
    "CreateNode": ["nodeid", "alias", "datatype", "nodegroup_id"],
    "DeleteNode": ["nodeid", "alias", "datatype"],
    "CreateNodeGroup": ["nodegroupid", "parent_nodegroup_id"],
    "DeleteNodeGroup": ["nodegroupid", "parent_nodegroup_id"],
    "AlterNodeGroupParent": [
        "nodegroupid",
...
class MigrationWriter:
    ...
    def as_string(self) -> str:
        ...
        lines = [
            f"# Generated by Arches compare_graph_publications on {now}",
            ...
            "",
            "from arches.app.graph_migrations import operations",
            "",
            "",
            "class GraphMigration:",
            f"    graph_slug = {self.graph_slug!r}",
            f"    from_publication = {str(self.pub_a.publicationid)!r}",
            f"    to_publication = {str(self.pub_b.publicationid)!r}",
            "",
            "    operations = [",
            ops_block,
            "    ]",
    ...
    def _render_op(self, op: dict) -> str:
        kind = op["op"]
        params = _OP_PARAMS.get(kind, [k for k in op if k != "op"])
        lines = [f"        operations.{kind}("]
        for param in params:
            value = op.get(param)

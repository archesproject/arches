...
class Command(BaseCommand):
    ...
    def update_yarn_depenencies(self, version):
        ...
        if self.overwite_existing_file(yarn_config_path, "A .yarnrc file already exists. \
        ...
        if self.overwite_existing_file(packages_path, "A package.json file already exists. \
                Would you like to overwrite it with the latest? \
                If you have not manually modified this file since creating this project,\
                the answer is probably 'yes'") is True:
        ...
        if utils.get_yn_input("Would you like to update your javascript packages using yarn? \
                Yarn v1.5.1 or greater is required") is True:
            try:

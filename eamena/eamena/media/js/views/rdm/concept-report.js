define([
    ...
    return Backbone.View.extend({
        events: {
            ...
            'click .concept-report-content *[data-action="delete-parent"]': 'deleteClicked',
        ...
        deleteClicked: function(e) {
            ...
            if (data.action === 'delete-concept' || data.action === 'delete-parent'){
        ...
        deleteConfirmed: function(e) {
            ...
            }else if (data.action === 'delete-parent'){
                modal.find('h4').text(' ' + title);
                modal.find('.modal-title').addClass('loading');    
                            
                model = new ConceptModel(data)
                self.model.set('subconcepts', [model]);
                self.model.set('delete_self', true);
                self.model.set('nodetype', 'ConceptScheme');
                self.model.set('id', '00000000-0000-0000-0000-000000000001');
                self.model.delete(function(){
                    modal.find('h4').text(title);
                    modal.find('.modal-title').removeClass('loading');
                    modal.modal('hide');
